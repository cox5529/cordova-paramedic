...
class PluginsManager {
    ...
    installSinglePlugin (plugin) {
        ...
        let plugAddCmd = exec(this.config.getCli() + ' plugin add ' + plugin);
        for (var i = 0; i < 3; i++) {
            if (plugAddCmd.code === 0) {
                break;
            }
            
            exec(this.config.getCli() + ' plugin rm ' + plugin);
            plugAddCmd = exec(this.config.getCli() + ' plugin add ' + plugin);
        }
    ...
    uninstallSinglePlugin (plugin) {
        const pluginRemoveCmd = exec(this.config.getCli() + ' plugin rm ' + plugin);
        if (pluginRemoveCmd.code !== 0) {
            logger.error('Failed to remove plugin : ' + plugin);
            throw new Error('Failed to remove plugin : ' + plugin);
        }
    }
