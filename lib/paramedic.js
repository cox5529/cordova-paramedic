...
class ParamedicRunner {
  ...
  injectReporters() {
    ...
    ['jasmineStarted', 'specStarted', 'specDone', 'suiteStarted', 'suiteDone', 'jasmineDone'].forEach((route) => {
  ...
  maybeRunFileTransferServer() {
    return Q().then(() => {
      ...
      for (let i = 0; i < plugins.length; i++) {
        if (
          plugins[i].indexOf('cordova-plugin-file-transfer') >= 0 &&
          !this.config.getFileTransferServer() &&
          !this.config.isCI()
        ) {
  ...
  runLocalTests() {
    ...
    return Q()
        ...
        return Q.all([
          Q().then(() => {
            ...
            ps.lookup(
              ...
              (err, resultList) => (runProcess = resultList[0])
            );

            logger.info('cordova-paramedic: Process launched');
          }),
          Q().then(() => {
            ...
            if (this.shouldWaitForTestResult()) {
              return Q.promise((resolve, reject) => {
                ...
                this.waitForTests().then(() => {
                  logger.info('cordova-paramedic: resolved');
                  resolve();
                });
              });
            }
      ...
      .then((results) => {
        logger.info('cordova-paramedic: results = ' + results);
        return results[1];
      })
      .fin((result) => {
        logger.info('cordova-paramedic: ' + runProcess);
        return runProcess
          ? Q.Promise((resolve) => {
  ...
  runAppiumTests(useSauce) {
    ...
    const options = {
      ...
      pluginRepos: this.config
        .getPlugins()
        .map((plugin) => path.join(this.tempFolder.name, 'plugins', path.basename(plugin))),
  ...
  runTests() {
    ...
    if (this.config.shouldUseSauce()) {
      return this.paramedicSauceLabs.runSauceTests
        ...
        .then(
          (isAppiumTestPassed) => isTestPassed === utilities.TEST_PASSED && isAppiumTestPassed === utilities.TEST_PASSED
        );
      // Not Sauce Labs
    } else {
  ...
  waitForTests() {
    ...
    return Q.promise((resolve, reject) => {
      ...
      const ERR_MSG =
        'waitForTests: Seems like device not connected to local server in ' +
  ...
  getCommandForStartingTests() {
    const cmd = [this.config.getCli(), this.config.getAction(), this.config.getPlatformId()]
    ...
    if (this.isBrowser) {
      ...
      this.config.getAction() === 'build' ||
      (this.isWindows && this.config.getArgs().indexOf('appx=8.1-phone') < 0)
    ) {
  ...
  waitForConnection() {
    const ERR_MSG =
      'waitForConnection: Seems like device not connected to local server in ' +
      INITIAL_CONNECTION_TIMEOUT / 1000 +
      ' secs';
  ...
  collectDeviceLogs() {
    ...
    const paramedicLogCollector = new ParamedicLogCollector(
  ...
  uninstallApp() {
    ...
    return paramedicAppUninstall.uninstallApp(this.targetObj, this.config.getAppName());
  }
}
